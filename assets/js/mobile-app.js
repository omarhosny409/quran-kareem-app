...
function cachedPages(){const items=Object.values(OFFLINE_PAGES);for(let i=0;i<localStorage.length;i++){const key=localStorage.key(i);if(key&&key.startsWith('qma-page-v14-clean-')){const data=readJson(key);if(data&&data.ayahs)items.push(data)}}const seen=new Set;return items.filter(p=>{if(seen.has(p.number))return false;seen.add(p.number);return true})}function escapeRegExp(v){return String(v).replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}function resultMarkup(result,query){const text=escapeHtml(result.text||'').replace(new RegExp(escapeRegExp(escapeHtml(query)),'gi'),'<mark>$&</mark>');return`<a class="qma-result-card" href="surah.html?page=${result.page||149}"><strong>${escapeHtml(result.title)}</strong><p>${text}</p></a>`}
